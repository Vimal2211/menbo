...
export class ServiceModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.modalService.getActiveModal().subscribe(modal => {
      ...
      if (modal) {
        setTimeout(() => {
          const modalBody = this.getModalBodyElement(modal);
          if (modalBody) {
  ...
  private getModalBodyElement(modalName: string): HTMLElement | null {
    return document.querySelector(`[data-modal="${modalName}"]`) as HTMLElement | null;
  }
